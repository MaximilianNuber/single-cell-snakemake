...
def pca_from_counts_scanpy(
    ...
    *,
    ...
    """Counts → (norm→log1p) → HVG → (scale) → PCA. Returns only small pieces."""
    ...
    sc.tl.pca(
        tmp, n_comps=n_comps, zero_center=zero_center,
    ...
    outputs = {
        ...
        "uns.pca":    {"variance": pca_uns.get("variance"),
                       "variance_ratio": pca_uns.get("variance_ratio")},
        "var.hvg_names": tmp.var_names.to_numpy(),                       # HVG list
        "varm.PCs":   tmp.varm.get("PCs"),                               # optional (genes x PCs)
    }
    state = {
        "target_sum": target_sum, "log1p": log1p, "n_hvgs": n_hvgs,
        "hvg_flavor": hvg_flavor, "scale_max": scale_max,
        "n_comps": n_comps, "zero_center": zero_center,
        "svd_solver": svd_solver, "random_state": random_state,
        "schema": "embedding/pca_scanpy@v1",
    }
    vr = outputs["uns.pca"].get("variance_ratio")
